...
export default function Component() {
  ...
  const renderHeader = () => (
    <div className="sticky top-0 z-10 bg-black/50 backdrop-blur-md p-2 rounded-full">
      <Card className="bg-transparent border-0 overflow-hidden">
        <CardContent className="p-2 flex items-center justify-between relative">
  ...
  const renderFooter = () => (
    <motion.div
      className="fixed bottom-0 left-0 right-0 bg-black/30 backdrop-blur-md p-4 rounded-t-3xl"
      initial={{ y: 50 }}
      animate={{ y: 0 }}
  ...
  const renderHome = () => (
    <div className="flex-grow flex flex-col items-center justify-center p-4 relative">
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className="relative w-80 h-80 rounded-full overflow-hidden shadow-lg"
        onClick={clickCoin}
      >
        <Image
    ...
    return (
      ...
      >
        ...
        <NeonGradientCard className="w-full max-w-4xl bg-gray-800 rounded-lg shadow-lg overflow-hidden">
          <div className="grid grid-cols-5 gap-4 p-4 bg-gray-700 text-white font-bold">
            <div>Rank</div>
            <div>Player</div>
            <div>Coins</div>
            <div>Profit/h</div>
              ...
              <motion.div
                key={player.id}
                className={`grid grid-cols-5 gap-4 p-4 ${index % 2 === 0 ? 'bg-gray-800' : 'bg-gray-900'} ${player.id === currentUserRank ? 'bg-indigo-900' : ''}`}
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.3, delay: index * 0.05 }}
              >
                <div className="flex items-center text-white">{player.id}</div>
                <div className="flex items-center text-white">
                  <Image
                    src={player.id <= 3 
                      ? `https://hebbkx1anhila5yf.public.blob.vercel-storage.com/PLAYER%20${player.id}%203D%20ICON-${player.id === 1 ? '3TrYn6NEWKVJFuSLX5T2WXkpSX0Osj' : player.id === 2 ? 'NvCADY4SVeRcNr66NptlBivT6r5aqq' : 'mCCNFMTXDUSwKREiHMa2biybwjzLEp'}.png`
                      : 'https://hebbkx1anhila5yf.public.blob.vercel-storage.com/PLAYER%203%203D%20ICON-mCCNFMTXDUSwKREiHMa2biybwjzLEp.png'
                  }
                  alt={`Player ${player.id} Icon`}
                  width={24}
                  height={24}
                  className="mr-2"
                />
              ...
              <div className="flex items-center justify-center">
                <Trophy className={`w-6 h-6 ${player.id <= 3 ? 'text-yellow-400' : 'text-gray-400'}`} />
              </div>
            </motion.div>
          ))}
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black overflow-hidden">
        ...
        >
          <Image
            src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Logo-yntKXKGx192nCRbcIvBj4TXx13y3Ji.png"
            alt="Logo"
            width={100}
            height={100}
